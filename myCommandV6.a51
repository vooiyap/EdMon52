;================================================================
; Use this version - vvy - 22/10/2024 @16:27
; This version has following commands:
; G - go command 
; M - display Memory command
; R - display Registers
; Ref:- http://ee6115.mit.edu/page/8051-r31jp-info.html
;================================================================
...
Start:						   ; stalls at 2nd row of data displayed. 
		...
		Lcall   initSerial     ; initialize hardware
		
		Lcall   printString    ; print welcome message
		db "AT89S52 monitor", 0h
			
monitorLoop:
		Clr     EA               ; disable all interrupts
		Clr     errorFlag        ; clear the error flag
		Lcall   printString            ; print prompt
		db 0Dh, 0Ah,">", 0h
		Clr     RI               ; flush the serial input buffer
		Lcall   getCommand       ; read the single-letter command
...
endLoop:                 	     ; come here after command has finished
		Sjmp 	monitorLoop      ; loop forever in monitor loop
...
;=================================================================
jumpTable:
		;dw DisplayMemory     ; command 'd' -> index 0
...
;*****************************************************************
;===============================================================
; Command m/M - M for Display memory
;===============================================================
DisplayMemory:
			Mov A,#" "
			Lcall sendCharacter			
			Lcall printString
			db " ", 0Dh, 0Ah, 0h
			Mov DPTR,#0000h		;Clear DPTR
			
Message:	Mov DPTR,#LUT
DoAgain:	Clr A
			Movc A,@A+DPTR
			Jz Begin
			Acall Transmit
			Inc DPTR
			Jmp DoAgain         
;============================ Display Memory Subroutine Starts Here ================================
;-- The following section get the memory address from user. It echoes what it is being keyed onto --
;-- the terminal. Secondly, it also stores the bytes of addresses into RAM memory. -----------------

Begin:      Mov R1,#20h             ;Address of array in data memory - store user input address from serial terminal
            Mov R7,#00h             ;Initialize counter - use to check no. of user inputs
            
HereAgain:  Acall getCharacter		;Get character
            Acall Echo				;Echo character
...
;----- This section convert byte data into hex address and display it ---------
            ...
            Mov A,@R1
            Acall ChkHiNib          ;Check high nibble
            Inc R1
            Mov A,@R1
            Acall ChkLoNib			;Check low nibble
            Inc R1
            Mov DPH, A
            Acall Hex2Ascii
            Acall DispAddr
...
;========================== Display one row of 16 data ===========================
Reload:     Mov R3,#10h		  ; R3 is used as a counter to check number of data
...
DispAscii:	Mov R3,#10h			; Counter to check no. of data (16 data)
...
;-------------------------End display ASCII Values--------------------------------					
;=================== End or displaying one row of data ===========================
...
; At this point, the next address id displayed - no hex values 	

;------------------------ Display next row data -----------------------------------
PointNext:  Inc R4
            Cjne R4,#0Ah,Reload		; goto Reload to display next row of data - 0Bh for 10 rows of data
            Sjmp EndHere
;------------------------- End of displaying row of data --------------------------            
EndHere:    
			Ljmp endLoop
			;Ret
;====================== Display Memory Subroutin Ends Here ========================
...
goCommand:	
			...
			Lcall prthex           ; display the high byte for debugging
			Lcall getByte          ; get address low byte
			Push  Acc              ; push LSB of jump address
			Lcall prthex           ; display the low byte for debugging
			Lcall crlf
...
transmitLoop:
		Jnb  SCON.1, transmitLoop    ; wait until chr is sent
		Ret
...
;===============================================================
getByte:
		Lcall getCharacter           ; get msb ascii chr
		Lcall ascii2Bin           ; conv it to binary
		Swap  A               ; move to most sig half of acc
		Mov   B,  A            ; save in b
		Lcall getCharacter           ; get lsb ascii chr
		Lcall ascii2Bin           ; conv it to binary
...
;====================== Subroutines Used in Main Program ==========================

;================================== Hex to Ascii ==================================
Hex2Ascii:  Mov B,A
            Anl A,#0Fh        ; mask upper nibble - work out lower nibble first				
            Acall H2ALow      ; Convert lower nibble hex to ASCII
			Mov A,B           ; get data into Acc
            Anl A,#0F0h       ; mask lower nibble 			
            Acall H2AUp	  	  ; Convert upper nibble hex to ASCII	
            Ret
;============================== End of Hex to Ascii ===============================

;========================== Converting High and Low Byte ==========================
H2ALow:		Cjne A,#0Ah,NotEqual			
...
HaltLo:		Ret 
;------------------------------------------------------------------------------------------
;----------------------------- Convert 0 to 9 to hex - high nibble ------------------------
Zero29ConH: 
            ...
            Mov R2,A
            Ret
;Convert 0 to 9 to hex - low nibble            
Zero29ConL: 
...
Skip:       Clr C
            ...
            Ret
;--------------------------------------------------------------------------------------------
;-------------------------- Convert A to F to hex - low nibble ------------------------------
A2FhConL:   
